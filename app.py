...
def home():
    ...
    db=Db(DATA_BASE)
    db.create_table(table_name='logins',cols=['user_id','password'])
    ...
    if request.method =='POST':
        ...
        if login:
            ...
            log(username).info(f'db:{DATA_BASE} connected successfuly')
            log(username).info(f'login for {username} with password {userpass}')
            log(username).info(message)

            db.create_table(table_name=table_name,cols=['dates datetime','time text','subject text','hour int','present int'])
